...
static void TFT_DisplayMenu(void)
{
  ...
  /* Display message */
  UTIL_LCD_DisplayStringAt(0,10, (uint8_t*)"  NUCLEO-H745ZI-Q   ", LEFT_MODE);
...
static void TFT_DisplayImages(void)
{
  ...
  uint32_t joystickstatus = JOY_NONE;
	uint32_t buttonstatus = BUTTON_NONE;
  ...
  while (1)
  {
    ...
    //joystickstatus = ADAFRUIT_802_JOY_GetState(JOY1);
		...
		/* Get Buttons status */
		buttonstatus = BSP_BUTTONS_GetState();
		
		if(buttonstatus){
			if(buttonstatus == BUTTON_A){
				printf("Button A pressed\r\n");
				BSP_LED_On(LED1);
			}else if(buttonstatus == BUTTON_B){
				printf("Button B pressed\r\n");
				BSP_LED_Off(LED1);
			}else if(buttonstatus == BUTTON_C){
...
static void SDCard_Config(void)
{
  ...
  if(FATFS_LinkDriver(&SD_Driver, SD_Path) == 0)
  {
	...
	printf("FS mount complete\r\n");
}

/**
  * @brief  SD Card Testing. Create on SD card file log.txt and write 
	* 				10 string "This text is written in the file.\n"
  * @param  None
  * @retval None
  */
static void SDCard_Test(void)
